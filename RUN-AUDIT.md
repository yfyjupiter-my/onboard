...
- [ ] RUN-003a prefix the last command with `exec`: `exec gunicorn onboard.wsgi:application …` (one word).
...
- [ ] RUN-004a add `conn_health_checks=True` to `dj_database_url.config(...)` in `settings.py` (dj-database-url 2.3.0 supports it; Django pings the connection once per request before reusing it).
...
- [ ] RUN-005a add `proxy_read_timeout 120s;` to `location /` in `nginx/default.conf.template` (matches `--timeout 120`).
...
Gate: **PASS**, no blocker. Three pending one-liners (RUN-003, RUN-004, RUN-005).
